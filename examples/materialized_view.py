...
class MaterializedViewExample:
    ...
    def __init__(self, dburi):
        ...
        self.settings = MaterializedViewSettings(database=DatabaseAddress.from_string(dburi))
        if "PYTEST_CURRENT_TEST" not in os.environ:
            self.settings.materialized_table.schema = "examples"
    ...
    def refresh(self):
        ...
        # self.db.run_sql('REFRESH TABLE "examples"."raw_metrics_view";')
...
def main(dburi: str):
    ...
    # Report about number of records in table after first materialized view refresh.
    ...
    # Report about number of records in table after second materialized view refresh.
    count = example.db.count_records("examples.raw_metrics_view")
    logger.info(f"Database table `examples.raw_metrics_view` contains {count} records")
